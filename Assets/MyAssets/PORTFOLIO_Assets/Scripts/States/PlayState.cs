namespace Assets.MyAssets.PORTFOLIO_Assets.Scripts.States
{
    ...
    ///
    /// Enter()가 새 판을 시작하는 지점이다. Pause에서 돌아올 때는 Pop이라
    /// Enter()가 다시 불리지 않으므로, 여기에 StartGame()을 두어도 게임이 재시작되지 않는다.
    /// (평면 FSM이었다면 여기서 재시작 버그가 났을 자리다.)
    ///
    /// 버튼 연결(인스펙터 onClick):
    ...
    public class PlayState : UiStateBase
    {
        ...
        public override void Enter()
        {
            ...
            //   점수·시간·성공/실패·쟁반·remaining 초기화 + 첫 손님 생성 + 이벤트 전부 발행
        }

        public override void Exit()
        {
            // TODO: gameState.SetRunning(false);
            //   Result로 가든 Title로 가든, 어느 경로로 나가도 시계가 확실히 멈추게 한다.
        }
