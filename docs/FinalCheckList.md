...
1. UI/UX 설계 문서
   ...
   - UI 리소스 규칙 (글꼴, 버튼 상태, 아이콘, 패널) 과 UI/UX 콘셉트를 한 문단으로 적습니다.

2. GUI 디자인 가이드와 구현 가능성 검토
   - 최소 버튼 크기, 글자 대비, 정보 우선순위, 지원 입력 방식을 표로 정리합니다.
   - 해상도 또는 입력 테스트에서 원안을 바꾼 항목이 있다면 원인과 변경 내용을 기록합니다.
...
8. 체크리스트 검증
   ...
   - UI 요소 2개 이상에 대해 기대 결과, 실제 결과, 수정 내용, 다시 확인한 결과를 기록합니다.
...
| UI 리소스 규칙과 UI/UX 콘셉트를 설계 문서에 적었다. |  |
| GUI 디자인 가이드와 구현 가능성 검토 결과를 남겼다. |  |
...
| 해상도 변경 테스트를 했다. |  |
| 구현 설명서와 체크리스트를 작성했다. |  |
| 테스트에서 발견한 문제와 설계 변경, 다시 확인한 결과를 기록했다. |  |
